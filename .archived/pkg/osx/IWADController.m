...
// Set the dropdown list to include an entry for each IWAD that has been configured.
// Returns true if at least one IWAD is configured.
- (BOOL) setDropdownList
{
	...
	// Enable/disable the dropdown depending on whether there were any configured IWADs.
...
- (void) saveConfig
{
	...
	selectedIWAD = [self getSelectedIWAD];
	[defaults setObject:IWADFilenames[selectedIWAD] forKey:@"selected_iwad"];
}

// Callback method invoked when the configuration button in the main window is clicked.
...
// Generate a value to set for the DOOMWADPATH environment variable that contains each of the configured IWAD files.
- (char *) doomWadPath
{
	NSPathControl *iwadList[NUM_IWAD_TYPES];
	NSString *location;
...
- (NSString *) autoloadPath
{
	...
	return [NSString pathWithComponents: [NSArray arrayWithObjects: base, @PACKAGE_TARNAME, @"autoload", IWADFilenames[selectedIWAD], nil]];
}

// Set the DOOMWADPATH environment variable to contain the path to each of the configured IWAD files.
- (void) setEnvironment
{
	char *doomwadpath;
	char *env;
...
- (BOOL) addIWADPath: (NSString *) path
{
	...
	// Find an IWAD file that matches the filename in the path that we have been given.
...
- (BOOL) selectGameByName: (const char *) name
{
	...
	// Already selected an IWAD of the desired type? Just return success.
	if (!strcmp(name, [self getGameName]))
	{
		return YES;
	}

	// Search through the configured IWADs and try to select the desired game.
	[self getIWADList: iwadList];

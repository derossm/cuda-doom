...
+ (void)initialize
{
	...
	// Register your app's defaults here by adding objects to the dictionary,
	// e.g. [defaults setObject:anObject forKey:keyForThatObject];
...
- (BOOL) application:(NSApplication *) application openFile:(NSString *) fileName
{
	...
	// This may be an IWAD. If so, add it to the IWAD configuration; don't add it like a PWAD.
	if ([self->launcherManager addIWADPath: fileName])
	{
		return YES;
	}

	// If this is the first file added, clear out the existing command line.
	// This allows us to select multiple files in the finder and open them all together (for TCs, etc).
	if (!self->filesAdded)
	{
	...
	{
		[self->launcherManager addFileToCommandLine: fileName forArgument: @"-merge"];
	}
	else if (![extension caseInsensitiveCompare: @"lmp"])
	{
		[self->launcherManager addFileToCommandLine: fileName forArgument: @"-playdemo"];
	}
	else if (![extension caseInsensitiveCompare: @"deh"])
	{
		[self->launcherManager addFileToCommandLine: fileName forArgument: @"-deh"];

...
using sha1_digest_t = ::std::byte[20];
...
// The routine final terminates the computation and returns the digest. The handle is prepared for a new cycle,
// but adding bytes to the handle will the destroy the returned buffer. Returns: 20 bytes representing the digest.

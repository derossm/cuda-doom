						...
						overRide)
{
	...
	pe = Z_Malloc<polyevent_t>(sizeof(polyevent_t), pu_tags_t::PU_LEVSPEC, 0);
...
bool EV_OpenPolyDoor(line_t * line, byte * args, podoortype_t type)
{
	...
	pd = Z_Malloc<polydoor_t>(sizeof(polydoor_t), pu_tags_t::PU_LEVSPEC, 0);
		...
		for (i = po->bbox[BOXLEFT]; i <= po->bbox[BOXRIGHT]; i++)
		{
				...
				&& j < bmapheight * bmapwidth)
			{
				...
				{				// Create a new link at the current block cell
					*link = Z_Malloc<polyblock_t>(sizeof(polyblock_t), pu_tags_t::PU_LEVEL, 0);
					(*link)->next = NULL;
					(*link)->prev = NULL;
					(*link)->polyobj = po;
					continue;
...
static void SpawnPolyobj(int index, int tag, bool crush)
{
	...
	{							// didn't find a polyobj through PO_LINE_START
		...
		if (polyobjs[index].numsegs)
		{
			...
			polyobjs[index].crush = crush;
			polyobjs[index].tag = tag;
			polyobjs[index].segs = Z_Malloc<decltype(polyobjs[index].segs)>(polyobjs[index].numsegs
											* sizeof(seg_t *), pu_tags_t::PU_LEVEL, 0);
...
static void TranslateToStartSpot(int tag, int originX, int originY)
{
	...
	po->originalPts = Z_Malloc<decltype(po->originalPts)>(po->numsegs * sizeof(vertex_t), pu_tags_t::PU_LEVEL, 0);
	po->prevPts = Z_Malloc<decltype(po->prevPts)>(po->numsegs * sizeof(vertex_t), pu_tags_t::PU_LEVEL, 0);
...
void PO_Init(int lump)
{
	...
	polyobjs = Z_Malloc<decltype(polyobjs)>(po_NumPolyobjs * sizeof(polyobj_t), pu_tags_t::PU_LEVEL, 0);

...
bool P_TeleportMove(mobj_t* thing, fixed_t x, fixed_t y)
{
	...
	auto xl = (tmbbox[BOXLEFT] - bmaporgx - MAXRADIUS)>>MAPBLOCKSHIFT;
	auto xh = (tmbbox[BOXRIGHT] - bmaporgx + MAXRADIUS)>>MAPBLOCKSHIFT;
	auto yl = (tmbbox[BOXBOTTOM] - bmaporgy - MAXRADIUS)>>MAPBLOCKSHIFT;
	auto yh = (tmbbox[BOXTOP] - bmaporgy + MAXRADIUS)>>MAPBLOCKSHIFT;
...
bool PIT_CheckLine(line_t* ld)
{
	...
	if (!(tmthing->flags & MF_MISSILE) )
	{
		...
		if (ld->flags & ML_BLOCKING && (!(ld->flags & ML_JUMPOVER) || tmfloorz + (32*FRACUNIT) > tmthing->z))
		{
			return false;	// explicitly blocking everything
		}

		// villsa [STRIFE] exclude floaters from blockmonster lines
		if ( !tmthing->player && (ld->flags & ML_BLOCKMONSTERS) && !(tmthing->flags & MF_FLOAT))
		{
...
bool PIT_CheckThing(mobj_t* thing)
{
	...
	if (tmthing->flags & MF_MISSILE)
	{
		...
		auto damage{((P_Random()&3)+1)*tmthing->info->damage};
	...
	{
		bool solid = (thing->flags & MF_SOLID) != 0;
...
bool P_TryMove(mobj_t* thing, fixed_t x, fixed_t y)
{
	...
	if ( !(thing->flags & MF_NOCLIP) )
	{
		...
		if (!(thing->flags&(MF_DROPOFF|MF_FLOAT)) && tmfloorz - tmdropoffz > 32*FRACUNIT)
		{
...
void P_UseLines(player_t* player)
{
	...
	auto x1{player->mo->x};
	auto y1{player->mo->y};
	fixed_t x2{x1 + (USERANGE>>FRACBITS)*finecosine[angle]};
	fixed_t y2{y1 + (USERANGE>>FRACBITS)*finesine[angle]};
...
//
// [STRIFE] Modified to emit "test" tracers which can shatter glass screens and windows.
void P_RadiusAttack(mobj_t* spot, mobj_t* source, int damage)
{
	fixed_t dist{(damage + MAXRADIUS)<<FRACBITS};
	auto yh{(spot->y + dist - bmaporgy)>>MAPBLOCKSHIFT};
	auto yl{(spot->y - dist - bmaporgy)>>MAPBLOCKSHIFT};
	auto xh{(spot->x + dist - bmaporgx)>>MAPBLOCKSHIFT};
	auto xl{(spot->x - dist - bmaporgx)>>MAPBLOCKSHIFT};
	...
	spot->z += 32*FRACUNIT;
	P_LineAttack(spot, (angle_t)0,	dist, 1, 0);
...
bool P_ChangeSector(sector_t* sector, bool crunch)
{
	...
	for (auto x{sector->blockbox[BOXLEFT]}; x <= sector->blockbox[BOXRIGHT]; ++x)
	{
		for (auto y{sector->blockbox[BOXBOTTOM]}; y <= sector->blockbox[BOXTOP]; ++y)
		{
...
static void SpechitOverrun(line_t* ld)
{
	static size_t baseaddr{0};

	if (baseaddr == 0)
	{
		// This is the first time we have had an overrun. Work out what base address we are going to use.
		...
		if (auto p{M_CheckParmWithArgs("-spechit", 1)}; p > 0)
		{
	...
	auto addr{baseaddr + (ld - lines) * 0x3E};

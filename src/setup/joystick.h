...
static txt::Button* joystick_button;
static txt::txt_joystick_axis_t* x_axis_widget;
static txt::txt_joystick_axis_t* y_axis_widget;

//
// Calibration
//

static txt::Window* calibration_window;
...
static const known_joystick_t* GetJoystickType(int index)
{
	...
	for (i = 0; i < arrlen(known_joysticks); ++i)
	{
		...
		{
			if (strncmp(known_joysticks[i].name, name, strlen(known_joysticks[i].name) - 1) != 0)
			{
				continue;
			}
		}
...
// We identify joysticks using GUID where possible, but joystick_index is used to distinguish between different devices. As the index can
// change, UpdateJoystickIndex() checks to see if it is still valid and updates it as appropriate.
static void UpdateJoystickIndex()
{
	...
	// If index is not correct, look for the first device with the expected GUID. It may have moved to a different index.
	...
	// Not found; it's possible the device is disconnected. Do not reset joystick_guid or joystick_index in case they are reconnected later.
}
...
static void SetJoystickButtonLabel()
{
	...
	txt::SetButtonLabel(joystick_button, (char*)name);
}
...
{
	txt::ConfigureJoystickAxis(x_axis_widget, calibrate_button, NULL);
}

// Given the SDL_JoystickID instance ID from a button event, set the joystick_guid and joystick_index config variables.
static bool SetJoystickGUID(SDL_JoystickID joy_id)
{
	...
	for (i = 0; i < all_joysticks_len; ++i)
	{
		if (SDL_JoystickInstanceID(all_joysticks[i]) == joy_id)
		{
			...
			joystick_guid = static_cast<decltype(joystick_guid)>(malloc(33));
...
static int CalibrationEventCallback(SDL_Event* event, void* user_data)
{
	...
	// In the first "center" stage, we're just trying to work out which joystick is being configured and which button the user is pressing.
	usejoystick = 1;
	calibrate_button = event->jbutton.button;

	// If the joystick is a known one, auto-load default config for it. Otherwise, proceed with calibration.
	if (IsKnownJoystick(joystick_index))
	{
		LoadKnownConfiguration();
		txt::CloseWindow(calibration_window);
	}
	else
	{
		txt::CloseWindow(calibration_window);

		// Calibrate joystick axes: Y axis first, then X axis once completed.
		txt::ConfigureJoystickAxis(y_axis_widget, calibrate_button,
			CalibrateXAxis);
	}
...
{
	txt::MessageBox(NULL, "No gamepads or joysticks could be found.\n\nTry configuring your controller from within\n"
...
static void CalibrateWindowClosed(txt::UNCAST_ARG(widget), txt::UNCAST_ARG(unused))
{
	txt::SDL_SetEventCallback(NULL, NULL);
	SetJoystickButtonLabel();
	CloseAllJoysticks();
}

static void CalibrateJoystick(txt::UNCAST_ARG(widget), txt::UNCAST_ARG(unused))
{
	...
	calibration_window = txt::NewWindow("Gamepad/Joystick calibration");

	txt::AddWidgets(calibration_window,
		txt::NewStrut(0, 1),
		txt::NewLabel("Center the D-pad or joystick,\nand press a button."),
		txt::NewStrut(0, 1),
		NULL);

	txt::SetWindowAction(calibration_window, txt::HORIZ_LEFT, NULL);
	txt::SetWindowAction(calibration_window, txt::HORIZ_CENTER,
		txt::NewWindowAbortAction(calibration_window));
	txt::SetWindowAction(calibration_window, txt::HORIZ_RIGHT, NULL);

	txt::SDL_SetEventCallback(CalibrationEventCallback, NULL);

	txt::SignalConnect(calibration_window, "closed", CalibrateWindowClosed, NULL);
...
// GUI
//
static void AddJoystickControl(txt::UNCAST_ARG(table), ::std::string label, int* var)
{
	txt::CAST_ARG(txt::txt_table_t, table);
	txt::txt_joystick_input_t* joy_input;

	joy_input = txt::NewJoystickInput(var);

	txt::AddWidgets(table, txt::NewLabel(label), joy_input, txt::TABLE_EMPTY, NULL);
}

void ConfigJoystick(txt::UNCAST_ARG(widget), void* user_data)
{
	txt::Window* window;

	window = txt::NewWindow("Gamepad/Joystick configuration");
	txt::SetTableColumns(window, 6);
	txt::SetColumnWidths(window, 18, 10, 1, 15, 10, 0);
	txt::SetWindowHelpURL(window, WINDOW_HELP_URL);

	txt::AddWidgets(window,
		txt::NewLabel("Controller"),
		joystick_button = txt::NewButton("zzzz"),
		txt::TABLE_EOL,

		txt::NewSeparator("Axes"),
		txt::NewLabel("Forward/backward"),
		y_axis_widget =
		txt::NewJoystickAxis(&joystick_y_axis, &joystick_y_invert, JOYSTICK_AXIS_VERTICAL),
		txt::TABLE_OVERFLOW_RIGHT,
		txt::TABLE_OVERFLOW_RIGHT,
		txt::TABLE_EMPTY,
		txt::TABLE_EMPTY,

		txt::NewLabel("Turn left/right"),
		x_axis_widget =
		txt::NewJoystickAxis(&joystick_x_axis, &joystick_x_invert, JOYSTICK_AXIS_HORIZONTAL),
		txt::TABLE_OVERFLOW_RIGHT,
		txt::TABLE_OVERFLOW_RIGHT,
		txt::TABLE_EMPTY,
		txt::TABLE_EMPTY,

		txt::NewLabel("Strafe left/right"),
		txt::NewJoystickAxis(&joystick_strafe_axis, &joystick_strafe_invert, JOYSTICK_AXIS_HORIZONTAL),
		txt::TABLE_OVERFLOW_RIGHT,
		txt::TABLE_OVERFLOW_RIGHT,
		txt::TABLE_EMPTY,
		txt::TABLE_EMPTY,
	...
	{
		txt::AddWidgets(window,
			txt::NewLabel("Look up/down"),
			txt::NewJoystickAxis(&joystick_look_axis, &joystick_look_invert, JOYSTICK_AXIS_VERTICAL),
			txt::TABLE_OVERFLOW_RIGHT,
			txt::TABLE_OVERFLOW_RIGHT,
			txt::TABLE_EMPTY,
			txt::TABLE_EMPTY,
	...
	txt::AddWidget(window, txt::NewSeparator("Buttons"));
	...
	txt::SignalConnect(joystick_button, "pressed", CalibrateJoystick, NULL);
...
void BindJoystickVariables()
{
	...
	for (i = 0; i < NUM_VIRTUAL_BUTTONS; ++i)
	{
		...
		M_BindVariable<int>(name, &joystick_physical_buttons[i]);
	}
}

...
//e6y
constexpr size_t STAIRS_UNINITIALIZED_CRUSH_FIELD_VALUE{10};
...
void T_MoveGoobers(floormove_t* floor)
{
	...
	res2 = T_MovePlane(floor->sector, 2 * FLOORSPEED, floor->floordestheight, true, 1, (floor->direction >> 1) * 2 - 1);
...
void EV_DoGoobers()
{
	for (size_t i{0}; i < numsectors; ++i)
	{
		...
		floormove_t* floor = Z_Malloc<decltype(floor)>(sizeof(*floor), pu_tags_t::PU_LEVSPEC, nullptr);
		P_AddThinker(static_cast<thinker_t*>(&floor->thinker));
	...
	while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
	{
		...
		for(bool ok{true}; ok; )
		{
			...
			for (size_t i{0}; i < sec->linecount; ++i)
			{
				...
				floor = Z_Malloc<decltype(floor)>(size_t(sizeof(*floor)), pu_tags_t::PU_LEVSPEC, (void*)nullptr);

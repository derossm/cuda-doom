...
mapformat_t P_CheckMapFormat(int lumpnum)
{
	...
	if ((b = lumpnum + ML_BLOCKMAP + 1) < numlumps && !strncasecmp(lumpinfo[b]->name, "BEHAVIOR", 8))
	{
		...
		format |= mapformat_t::MFMT_HEXEN;
	}
	else
		fprintf(stderr, "Doom (");

	if (!((b = lumpnum + ML_NODES) < numlumps && (nodes = W_CacheLumpNum<byte>(b, pu_tags_t::PU_CACHE)) && W_LumpLength(b) > 0))
		fprintf(stderr, "no nodes");
	else if (!memcmp(nodes, "xNd4\0\0\0\0", 8))
	{
		fprintf(stderr, "DeePBSP");
		format |= mapformat_t::MFMT_DEEPBSP;
	}
	else if (!memcmp(nodes, "XNOD", 4))
	{
		fprintf(stderr, "ZDBSP");
		format |= mapformat_t::MFMT_ZDBSPX;
	}
	else if (!memcmp(nodes, "ZNOD", 4))
	{
		fprintf(stderr, "compressed ZDBSP");
		format |= mapformat_t::MFMT_ZDBSPZ;
	}
	else
		fprintf(stderr, "BSP");
...
void P_LoadSegs_DeePBSP(int lump)
{
	...
	data = W_CacheLumpNum<mapseg_deepbsp_t>(lump, pu_tags_t::PU_STATIC);
...
void P_LoadNodes_ZDBSP(int lump, bool compressed)
{
	...
	data = W_CacheLumpNum<byte>(lump, pu_tags_t::PU_LEVEL);
	...
	{
		newvertarray = Z_Malloc<decltype(newvertarray)>((orgVerts + newVerts) * sizeof(vertex_t), pu_tags_t::PU_LEVEL, 0);

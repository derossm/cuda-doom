...
int* texturewidth; // texture width for wrapping column getter function
// needed for texture pegging
fixed_t* textureheight; // texture height for Tutti-Frutti fix
int* texturecompositesize;
short** texturecolumnlump;
unsigned** texturecolumnofs; // column offsets for composited translucent mid-textures on 2S walls
unsigned** texturecolumnofs2; // column offsets for composited opaque textures
byte** texturecomposite; // composited translucent mid-textures on 2S walls
byte** texturecomposite2; // composited opaque textures
byte** texturebrightmap; // brightmaps
...
// replace R_DrawColumnInCache(), R_GenerateComposite() and R_GenerateLookup()
...
void R_GenerateComposite(int texnum)
{
	...
	// memory block for opaque textures
	...
	// initialize composite background to palette index 0 (usually black)
	...
	for (i = 0, patch = texture->patches;
		...
		++i, ++patch)
	{
		realpatch = W_CacheLumpNum<patch_t>(patch->patch, pu_tags_t::PU_CACHE);
		x1 = patch->originx;
		...
		for (; x < x2; ++x)
		{
			...
			// generate composites for single-patched columns as well
			/*
			if (collump[x] >= 0)
			continue;
			*/
			...
			R_DrawColumnInCache(patchcol,
				block + colofs[x],
				// single-patched columns are normally not composited
	...
	{
		// generate composites for all columns
	//	if (collump[i] == -1) // process only multipatched columns
		{
			column_t* col = (column_t*)(block + colofs[i] - 3); // cached column
			const byte* mark = marks + i * texture->height;
			int j = 0;
			// absolut topdelta for first 254 pixels, then relative
			int abstop;
			int reltop = 0;
			bool relative = false;
			...
			// copy composited columns to opaque texture
			...
			for (; ; ) // reconstruct the column by scanning transparency marks
			{
				...
				// once we pass the 254 boundary, topdelta becomes relative
				if ((abstop = j) >= 254)
				{
					relative = true;
					reltop = 0;
...
void R_GenerateLookup(int texnum)
{
	...
	// generate composites for all textures
// if (texture->patchcount > 1 && texture->height < 256)
	...
	for (x = 0; x < texture->width; ++x)
	{
		...
		{
			// fix absurd texture name in error message
		...
		// treat patch-less columns the same as multi-patched
		if (patchcount[x] > 1 || !patchcount[x])
		{
			// Use the cached block.
			// moved up here, the rest in this loop
		...
		*/
		...
		// initialize opaque texture column offset
...
// wrapping column getter function for composited translucent mid-textures on 2S walls
...
static void GenerateTextureHashTable()
{
	...
	textures_hashtable = Z_Malloc<texture_t>(sizeof(texture_t*) * numtextures, pu_tags_t::PU_STATIC, 0);
...
//
// partly rewritten to merge PNAMES and TEXTURE1/2 lumps
void R_InitTextures()
{
	...
	// allocate memory for the pnameslumps and texturelumps arrays
	...
	// make sure the first available TEXTURE1/2 lumps
	...
	// fill the arrays with all available PNAMES lumps
	...
	for (i = numlumps - 1; i >= 0; --i)
	{
		if (!strncasecmp(lumpinfo[i]->name, DEH_String("PNAMES"), 6))
		{
			...
			pnameslumps[numpnameslumps].names = W_CacheLumpNum<void>(pnameslumps[numpnameslumps].lumpnum, pu_tags_t::PU_STATIC);
			pnameslumps[numpnameslumps].nummappatches = LONG(*((int*)pnameslumps[numpnameslumps].names));

			// accumulated number of patches in the lookup tables
		...
		{
			// support only TEXTURE1/2 lumps, not TEXTURE3 etc.
			if (lumpinfo[i]->name[7] != '1' && lumpinfo[i]->name[7] != '2')
				continue;

			// make sure the first available TEXTURE1/2 lumps
			// are not processed again
			if (i == texturelumps[0].lumpnum || i == texturelumps[1].lumpnum) // may still be -1
				continue;
			...
			// do not proceed any further, yet
	...
	// fill up the patch lookup table
	name[8] = 0;
	...
	for (i = 0, k = 0; i < numpnameslumps; ++i)
	{
		for (j = 0; j < pnameslumps[i].nummappatches; ++j)
		{
			...
			// prevent flat lumps from being mistaken as patches
			while (p >= firstflat && p <= lastflat)
			{
				p = W_CheckNumForNameFromTo(name, p - 1, 0);
			}
			// if the name is unambiguous, use the lump we found
	...
	// release memory allocated for patch lookup tables
	...
	// pointer to (i.e. actually before) the first texture file
	texturelump = texturelumps - 1; // gets immediately increased below
	...
	if (I_ConsoleStdout())
	{
		...
		for (i = 0; i < temp3 + 9 + 1; ++i) // one more for R_InitTranMap()
#else
		for (i = 0; i < temp3 + 9; ++i)
#endif
			printf(" ");
		printf("]");
#ifndef CRISPY_TRUECOLOR
		for (i = 0; i < temp3 + 10 + 1; ++i) // one more for R_InitTranMap()
#else
	...
	for (i = 0; i < numtextures; ++i, ++directory)
	{
		...
		// initialize for the first texture file lump,
		...
		{
			// start looking in next texture file
			++texturelump;
		...
		texture = textures[i] = Z_Malloc<texture_t>(sizeof(texture_t) + sizeof(texpatch_t) * (SHORT(mtexture->patchcount) - 1), pu_tags_t::PU_STATIC, 0);
		...
		// texture width for wrapping column getter function
...
void R_InitSpriteLumps()
{
	...
	for (i = 0; i < numspritelumps; ++i)
	{
		...
		patch = W_CacheLumpNum<patch_t>(firstspritelump + i, pu_tags_t::PU_CACHE);
...
static void R_InitTranMap()
{
	...
	if (lump != -1)
	{
		...
		tranmap = W_CacheLumpNum<byte>(lump, pu_tags_t::PU_STATIC);
		// loaded from a lump
		printf(":");
	}
	else
	{
		...
		unsigned char* playpal = W_CacheLumpName<unsigned char>("PLAYPAL", pu_tags_t::PU_STATIC);
		FILE* cachefp;
		...
		fname = ::std::string(configdir + "tranmap.dat");

		// open file readable
		if ((cachefp = fopen(fname, "rb")) &&
			// could read struct cache from file
			fread(&cache, 1, sizeof(cache), cachefp) == sizeof(cache) &&
			// same filter percents
			cache.pct == tran_filter_pct &&
			// same base palettes
			memcmp(cache.playpal, playpal, sizeof(cache.playpal)) == 0 &&
			// could read entire translucency map
		...
		else
		{
			...
			for (i = 0; i < 256; ++i)
			{
				...
				for (j = 0; j < 256; ++j)
				{
					...
					btmp = fg[::std::size_t(rgb_t::b)] * 1.666 < (fg[::std::size_t(rgb_t::r)] + fg[::std::size_t(rgb_t::g)]) ? 0 : 50;
					blend[::std::size_t(rgb_t::r)] = (tran_filter_pct * fg[::std::size_t(rgb_t::r)] + (100 - tran_filter_pct) * bg[::std::size_t(rgb_t::r)]) / (100 + btmp);
					blend[::std::size_t(rgb_t::g)] = (tran_filter_pct * fg[::std::size_t(rgb_t::g)] + (100 - tran_filter_pct) * bg[::std::size_t(rgb_t::g)]) / (100 + btmp);
					blend[::std::size_t(rgb_t::b)] = (tran_filter_pct * fg[::std::size_t(rgb_t::b)] + (100 - tran_filter_pct) * bg[::std::size_t(rgb_t::b)]) / 100;

					*tp = I_GetPaletteIndex(blend[::std::size_t(rgb_t::r)], blend[::std::size_t(rgb_t::g)], blend[::std::size_t(rgb_t::b)]);
					++tp;
				}
			}

			// file not readable, open writable
...
void R_InitColormaps()
{
	...
	playpal = W_CacheLumpName<byte>("PLAYPAL", pu_tags_t::PU_STATIC);
	...
	{
		byte* const colormap = W_CacheLumpName<byte>("COLORMAP", pu_tags_t::PU_STATIC);
	...
	// initialize color translation and color strings tables
	{
		...
		// check for status bar graphics replacements
		i = W_CheckNumForName(DEH_String("sttnum0")); // Status Bar '0'
		keepgray = (i >= 0 && W_IsIWADLump(lumpinfo[i]));

		// CRMAX - 2: don't override the original GREN and BLUE2 Boom tables
		for (i = 0; i < CRMAX - 2; ++i)
		{
			for (j = 0; j < 256; ++j)
			{
...
void R_InitData()
{
	// Moved R_InitFlats() to the top, because it sets firstflat/lastflat
	...
	R_InitColormaps();
#ifndef CRISPY_TRUECOLOR
	R_InitTranMap(); // prints a mark itself
#endif
}
...
int R_FlatNumForName(::std::string name)
{
	...
	if (i == -1)
	{
		...
		// since there is no "No Flat" marker,
...
void R_PrecacheLevel()
{
	...
	// Precache flats.
	CHAR_PTR flatpresent = Z_Malloc<decltype(flatpresent)>(numflats, pu_tags_t::PU_STATIC, NULL);
	...
	// Precache textures.
	CHAR_PTR texturepresent = Z_Malloc<decltype(texturepresent)>(numtextures, pu_tags_t::PU_STATIC, nullptr);
	...
	// Note that finalestage_t::F_SKY1 is the name used to
	...
	// Precache sprites.
	CHAR_PTR spritepresent = Z_Malloc<decltype(spritepresent)>(numsprites, pu_tags_t::PU_STATIC, nullptr);
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		...
		{
			spritepresent[::std::size_t(((MapObject*)th)->sprite)] = 1;
		}
	}
	...
	for (size_t i{0}; i < numsprites; ++i)
	{
		...
		for (size_t j{0}; j < sprites[i].numframes; ++j)
		{

...
// Called on startup to save the location of the launcher program (within a package, other executables should be in the same directory)
...
// Execute the specified executable contained in the same directory as the launcher, with the specified arguments.
void ExecuteProgram(const char *executable, const char *iwad, const char *args)
{
	...
	if (childpid == 0)
	{
		...
		// Change directory to home dir before launch, so that any demos are saved somewhere sensible.
		homedir = getenv("HOME");
...
// Write a sequence of commands that will display the specified message via shell commands.
...
// Open a terminal window with the PATH set appropriately, and DOOMWADPATH set to the specified value.
void OpenTerminalWindow(const char *doomwadpath)
{
	...
	[[NSWorkspace sharedWorkspace] openFile: @TEMP_SCRIPT withApplication: @"Terminal"];
}
...
void OpenDocumentation(const char *filename)
{
	...
	path = [NSString stringWithFormat: @"%s/Documentation/%s", executable_path, filename];

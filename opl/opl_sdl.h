...
class OPLManager
{
	opl_callback_queue_t* callback_queue{nullptr};	// Queue of callbacks waiting to be invoked.
	SDL_mutex* callback_mutex{nullptr};				// When the callback mutex is locked using OPL_Lock, callback functions are not invoked.
	SDL_mutex* callback_queue_mutex{nullptr};		// Mutex used to control access to the callback queue.
	...
	opl3_chip opl_chip;								// OPL software emulator structure.
	...
	void AdvanceTime(uint64_t nsamples)
	{
		...
		// Advance time.
		us = (nsamples * OPL_SECOND) / mixing_freq;
		current_time += us;
		...
		while (!OPL_Queue_IsEmpty(callback_queue) && current_time >= OPL_Queue_Peek(callback_queue) + pause_offset)
		{
	...
	void FillBuffer(uint8_t* buffer, unsigned nsamples)
	{
	...
	void OPL_Mix_Callback(void* udata, Uint8* buffer, int len)
	{
		unsigned filled;
	...
	int OPL_SDL_Init(unsigned port_base)
	{
		...
		//Mix_SetPostMix(OPL_Mix_Callback, nullptr); FIXME

		return 1;
	}

	unsigned OPL_SDL_PortRead(opl_port_t port)
	{
		unsigned result{0};
	...
	void OPL_SDL_PortWrite(opl_port_t port, unsigned value)
	{
		if (port == opl_port_t::OPL_REGISTER_PORT)
		{
			register_num = value;
		}
		else if (port == opl_port_t::OPL_REGISTER_PORT_OPL3)
		{
			register_num = value | 0x100;
		}
		else if (port == opl_port_t::OPL_DATA_PORT)
		{
			WriteRegister(register_num, value);
		}
	}

	void OPL_SDL_SetCallback(uint64_t us, opl_callback_t callback, delay_data_t* data)
	{
		SDL_LockMutex(callback_queue_mutex);

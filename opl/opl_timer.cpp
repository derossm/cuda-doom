...
static int CallbackWaiting(uint64_t* next_time)
{
	...
	// Update pause_offset so after we unpause, the callback times will be right.
	if (opl_timer_paused)
	{
	...
	// If the time for the callback has not yet arrived, we must sleep until the callback time.

/**********************************************************************************************************************************************\
	...
	As with other Win32 modules, don't include most DOOM headers into this or conflicts will occur.
...
DIR* opendir(const _TCHAR* szPath)
{
	...
	// Initialize the dirent structure. ino and reclen are invalid under Win32, and name simply points at the appropriate part of the findfirst_t structure.
	nd->dd_dir.d_ino = 0;
...
dirent* readdir(DIR* dirp)
{
	...
	if (dirp->dd_stat < 0)
	{
		// We have already returned all files in the directory (or the structure has an invalid dd_stat).
		return (dirent*)0;
	}
	...
	if (dirp->dd_stat > 0)
	{
		// Successfully got an entry. Everything about the file is already appropriately filled in except the length of the file name.
...
// Return to the beginning of the directory "stream". We simply call findclose and then reset things like an opendir.
...
// Seek to an entry previously returned by telldir. We rewind the directory and call readdir repeatedly until either dd_stat is the position number
// or -1 (off the end). This is not perfect, in that the directory may have changed while we weren't looking. But that is probably the case with
// any such system.
